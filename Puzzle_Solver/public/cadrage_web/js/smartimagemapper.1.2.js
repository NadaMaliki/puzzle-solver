...
smartImageMap.module = (function(options) {
  ...
  var handlerCreateOutput = function() {
    return function(e) {
      ...
      theHtml =
        '<div class="draghandle"><h2>' +
        translate("Ausgabe") +
        " [" +
        format +
        ']<span id="closemap">X</span></h2></div><textarea name="output" id="output" class="output"></textarea>';
      oContainer
        .html("")
        .append(theHtml)
        .show();
  ...
  var handlerPolygonHandleMousemove = function(point, hotspotObj) {
    ...
    return function(e) {
      ...
      for (var i = 0; i < hotspotObj.shape.coords.length; i++) {
        if (
          oldX === hotspotObj.shape.coords[i] &&
          oldY === hotspotObj.shape.coords[i + 1]
        ) {
    ...
    return function(e) {
      ...
      for (var i = 0; i < points.length; i++) {
        if (isEven(i)) {
          ...
          points[i + 1] = parseInt(
            points[i + 1] + this.getAbsolutePosition().y
          );
        }
      }
  ...
  var handlerCircleMousemove = function(point, hotspotObj) {
    return function(e) {
      ...
      hotspotObj.shape.coords[2] = linedistance(
        centerX,
        centerY,
        point.x(),
        point.y()
    ...
    imageObj.onload = function() {
      ...
      if (imageObj.width >= settings.maximumImageSide) {
        minificationFactor = parseFloat(
          originalImageWidth / settings.maximumImageSide
        );
      ...
      if (imageObj.height >= settings.maximumImageSide) {
        minificationFactor = parseFloat(
          originalImageHeight / settings.maximumImageSide
        );
        minificationFactor = minificationFactor.toFixed(2);
  ...
  var createNewRectangle = function() {
    ...
    stage.on("mousemove", function() {
      var handleX, handleY, width, height;
  ...
  var drawShape = function(hotspotObj, targetLayer) {
    ...
    returnShape.on(
      ...
      handlerShape(hotspotObj, returnShape)
    );
    returnShape.on(
      "mouseup touchend",
      handlerShapeMouseup(hotspotObj, returnShape)
    );
  ...
  var displayPopertyPanel = function(hotspotObj) {
    ...
    //   "<tr><td><label>" +
    //     translate("Link") +
    //     ':</label></td><td><input type="text" name="link" id="link" value="' +
    //     myLink +
    //     '"/></td></tr>'
    // );
    // ptable.append(
    //   "<tr><td><label>" +
    //     translate("Target") +
    //     ':</label></td><td><input type="text" name="target" id="target" value="' +
    //     hotspotObj.target +
    //     '"/></td></tr>'
    // );
    ptable.append(
    ...
    //   "<tr><td><label>" +
    //     translate("Mouseover") +
    //     ':</label></td><td><textarea name="mouseover" id="mouseover">' +
    //     hotspotObj.events.mouseover +
    //     "</textarea></td></tr>"
    // );
    // ptable.append(
    //   "<tr><td><label>" +
    //     translate("Mouseout") +
    //     ':</label></td><td><textarea name="mouseout" id="mouseout">' +
    //     hotspotObj.events.mouseout +
    //     "</textarea></td></tr>"
    // );
    // ptable.append(
    //   "<tr><td><label>" +
    //     translate("Klick") +
    //     ':</label></td><td><textarea name="click" id="click">' +
    //     hotspotObj.events.click +
    //     "</textarea></td></tr>"
    // );
    ptable.append(
  ...
  var clearPopertyPanel = function() {
    var elProperty, elOutput, elOutputSVG;
  ...
  var drawHandle = function(x, y, hotspotObj) {
    ...
    if (shape == "polygon") {
      point.on(
        "dblclick",
        handlerDblClickDeletePointFromPolygon(point, hotspotObj)
      );

      point.on("mousedown touchstart", function() {
        stage.on(
          "mousemove touchmove",
          handlerPolygonHandleMousemove(point, hotspotObj)
        );
      });
      point.on(
        "dragend touchend",
        handlerPolygonHandleDragend(point, hotspotObj)
      );
    }
    ...
    if (shape == "rectangle") {
      ...
      point.on("mousedown touchstart", function() {
        stage.on(
          "mousemove touchmove",
          handlerRectangleHandleMousemove(point, hotspotObj)
        );
      });
      point.on(
        "dragend touchend",
        handlerRectangleHandleDragend(point, hotspotObj)
      );
    }
    ...
    if (shape == "circle") {
      ...
      point.on("mousedown touchstart", function() {
        stage.on(
          "mousemove touchmove",
          handlerCircleMousemove(point, hotspotObj)
        );
      });
      point.on(
        "dragend touchend",
        handlerCircleHandleDragend(point, hotspotObj)
      );
    }
  ...
  var addPointToPolygon = function(hotspotObj) {
    ...
    if (hotspotObj.shape.coords.indexOf(lastX) > -1) {
      if (
        hotspotObj.shape.coords.indexOf(lastX) + 1 ==
        hotspotObj.shape.coords.indexOf(lastY)
      ) {
  ...
  var getDistancePointLine = function(xa, ya, xb, yb, xp, yp) {
    ...
    var retval;
    retval =
      ((ya - yb) * xp + (xb - xa) * yp + (xa * yb - xb * ya)) /
      Math.sqrt((ya - yb) * (ya - yb) + (xb - xa) * (xb - xa));
  ...
  var createoutput = function(format) {
    var elTarget, strOutput, strArea, mapgiud, w, h;
    ...
    if (format === "html") {
      for (var i = 0; i < imagemapObj.imagemap.hotspots.length; i++) {
        ...
        var outputCords = coordinateCorrection(
          minificationFactor,
          imagemapObj.imagemap.hotspots[i].shape.coords
        );
        ...
        //   strArea +=
        //     ' target="' + imagemapObj.imagemap.hotspots[i].target + '"';
        // }
        // if (imagemapObj.imagemap.hotspots[i].events.mouseover) {
        //   strArea +=
        //     ' onmouseover="' +
        //     imagemapObj.imagemap.hotspots[i].events.mouseover +
        //     '"';
        // }
        // if (imagemapObj.imagemap.hotspots[i].events.mouseout) {
        //   strArea +=
        //     ' onmouseout="' +
        //     imagemapObj.imagemap.hotspots[i].events.mouseout +
        //     '"';
        // }
        // if (imagemapObj.imagemap.hotspots[i].events.click) {
        //   strArea +=
        //     ' onclick="' + imagemapObj.imagemap.hotspots[i].events.click + '"';
        // }
        strArea += "/&gt;\n";
      }
    ...
    if (format === "svg") {
      for (var i = 0; i < imagemapObj.imagemap.hotspots.length; i++) {
        ...
        var shape = getShapeTypeFromArrayLength(
          imagemapObj.imagemap.hotspots[i].shape.coords
        );
        strArea +=
          '&lt;a xlink:href="' + imagemapObj.imagemap.hotspots[i].link + '"';
        ...
        if (shape == "rectangle") {
          strArea +=
            ...
            outputCords[1] +
            '" width="' +
            parseInt(outputCords[2] - outputCords[0]) +
            '" height="' +
            parseInt(outputCords[3] - outputCords[1]) +
            '" fill="transparent" stroke="transparent"/&gt;\n';
        }
        if (shape == "polygon") {
          polypoints = "";
  ...
  var createHTMLContainer = function(elContainer) {
    ...
    html +=
      '<a href="javascript:void(0)" class="btnmode selection" id="selection" title="Selection Mode"></a>';
    html +=
      '<a href="javascript:void(0)" class="btnmode drawpolygon" id="drawpolygon" title="Draw polygon"></a>';
    html += "</div>";
    ...
    html +=
      '<textarea id="imagemap" class="hotspotsarea" name="json"></textarea>';
