...
function PiecesGallery({ imageId }) {
    ...
    const [grid, setGrid] = useState(Array.from({ length: 36 }, () => null));
    ...
    useEffect(() => {
        fetch(`http://localhost:5004/api/relatedData36/${imageId}`)
    ...
    const clearGrid = () => {
        setGrid(Array.from({ length: 36 }, () => null));
